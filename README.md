...
### Automatic download
When the API starts, it will automatically download the pretrained weights into
`models/pretrained/` if they are missing. Set `VOXGUARD_SKIP_DOWNLOAD=1` to
disable this behavior.

### Download helper
You can also pull the pretrained weights manually with:
```bash
python scripts/download_models.py
```

If you want to verify checksums locally after download:
```bash
